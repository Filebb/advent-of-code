...
data_action <- instructions |> 
    ...
    mutate(n_on = map_int(grid, sum))

total_lights_on <- data_action |> tail(1) |> pull(n_on)
...
data_action |> 
    ggplot(aes(x = seq_along(n_on), y = n_on)) +
    geom_line() +
...
total_brightness <- data_new_action |> tail(1) |> pull(brightness)
