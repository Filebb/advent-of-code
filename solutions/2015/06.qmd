...
instructions <- tibble(raw = input) |> 
    mutate(instruction = str_extract(raw, pattern = "toggle|turn on|turn off"),
           position    = str_extract_all(raw, pattern = "[0-9]+")) |> 
...
total_lights_on <- instructions |> tail(1) |> pull(n_on)
...
total_brightness <- instructions |> tail(1) |> pull(brightness)
